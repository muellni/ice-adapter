...
static void stunserver_udp_recv(const struct sa *src, struct mbuf *mb,
        void *arg)
{
  struct stunserver *stun = static_cast<struct stunserver *>(arg);
...
static void tcp_recv(struct mbuf *mb, void *arg)
{
  ...
  if (stun->mb) {
    ...
    stun->mb = static_cast<mbuf *>(mem_ref(mb));
  }
  ...
  for (;;) {
    ...
    if (stun->mb->pos >= stun->mb->end) {
      stun->mb = static_cast<mbuf *>(mem_deref(stun->mb));
      break;
    }
  }
...
static void tcp_close(int err, void *arg)
{
  ...
  stun->tc = static_cast<tcp_conn*>(mem_deref(stun->tc));
}
...
int stunserver_alloc(struct stunserver **stunp)
{
  ...
  stun = static_cast<struct stunserver *>(mem_zalloc(sizeof(*stun), stunserver_destructor));
  if (!stun)
    return ENOMEM;

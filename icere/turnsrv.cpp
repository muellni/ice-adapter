...
static struct turnserver::channel *find_channel_numb(struct turnserver *tt, uint16_t nr)
{
  size_t i;
...
static struct turnserver::channel *find_channel_peer(struct turnserver *tt,
...
{
  struct turnserver *turn = static_cast<struct turnserver *>(arg);
  struct turnserver::channel *chan;
  int err = 0;
...
{
  struct turnserver *conn = static_cast<struct turnserver *>(arg);
  int err = 0;
  ...
  if (conn->mb) {
    ...
    conn->mb = static_cast<mbuf *>(mem_ref(mb));
  }
  ...
  for (;;) {
    ...
    if (conn->mb->pos >= conn->mb->end) {
      conn->mb = static_cast<mbuf *>(mem_deref(conn->mb));
      break;
    }
  }
...
static void tcp_close_handler(int err, void *arg)
{
  ...
  turn->tc = static_cast<tcp_conn *>(mem_deref(turn->tc));
}
...
int turnserver_alloc(struct turnserver **turnp)
{
  ...
  turn = static_cast<struct turnserver *>(mem_zalloc(sizeof(*turn), destructor));
  if (!turn)
    return ENOMEM;

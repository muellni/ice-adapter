...
static int attr_add(struct attrs *attrs, const char *name,
        const char *value, ...)
{
  struct attrs::attr *attr = &attrs->attrv[attrs->attrc];
  va_list ap;
  int r, err = 0;
...
static const char *attr_find(const struct attrs *attrs, const char *name)
{
  ...
  for (i=0; i<attrs->attrc; i++) {
    const struct attrs::attr *attr = &attrs->attrv[i];
...
{
  struct agent *agent = static_cast<struct agent *>(arg);
...
static int agent_encode_sdp(struct agent *ag)
{
  ...
  for (le = icem_lcandl(ag->icem)->head; le; le = le->next) {
    ...
    struct cand *cand = static_cast<struct cand *>(le->data);
...
static int agent_decode_sdp(struct agent *agent, struct agent *other)
{
  ...
  for (i=0; i<other->attr_s.attrc; i++) {
    struct attrs::attr *attr = &other->attr_s.attrv[i];
  ...
  for (i=0; i<other->attr_m.attrc; i++) {
    struct attrs::attr *attr = &other->attr_m.attrv[i];
           ...
           const char *name, uint8_t compid, bool offerer)
{
  ...
  agent = static_cast<struct agent *>(mem_zalloc(sizeof(*agent), agent_destructor));
  if (!agent)
    return ENOMEM;
...
{
  struct ice_test *it = static_cast<struct ice_test *>(arg);
       ...
       enum ice_mode mode_b, bool turn_b)
{
  ...
  it = static_cast<struct ice_test *>(mem_zalloc(sizeof(*it), icetest_destructor));
  if (!it)
    return ENOMEM;
